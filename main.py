...
def apply_crt_filter(filepath, scale = 1, scanline_interval = 3):
    ...
    # Apply blur
    size = 5
    img_crt = cv2.GaussianBlur(img_crt, (size,size),13)
    # Apply horizontal scanlines
    for i in range(height//scanline_interval):
        img_crt = cv2.line(img_crt, (0,i*scanline_interval+scanline_interval//2), (width,i*scanline_interval+scanline_interval//2),(25,0,0), 1)
...
for image_path in os.listdir('assets\png'):
    apply_crt_filter('assets/png/'+image_path,1,4)
